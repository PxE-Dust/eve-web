...
/* ---------------- SWINGING ANIMATED VINES ---------------- */

function DynamicVines() {
  const { scrollY } = useScroll();
  
  // Transform scroll position into swinging angles with physics smooth spring
  const rawRotateLeft = useTransform(scrollY, [0, 2000], [0, 24]);
  const rawRotateRight = useTransform(scrollY, [0, 2000], [0, -24]);

  const rotateLeft = useSpring(rawRotateLeft, { stiffness: 40, damping: 12 });
  const rotateRight = useSpring(rawRotateRight, { stiffness: 40, damping: 12 });

  return (
    <div className="pointer-events-none fixed top-0 inset-x-0 h-80 z-20 overflow-hidden opacity-60">
      {/* Left Hanging Vine Branch */}
      <motion.div
        style={{ rotate: rotateLeft, transformOrigin: "top left" }}
        className="absolute top-0 left-0 w-64 h-80"
      >
        <svg className="w-full h-full text-[#1C2E20]" viewBox="0 0 200 250" fill="none" stroke="currentColor">
          <path d="M 0 0 C 40 60, 20 120, 50 200" strokeWidth="1.8" strokeLinecap="round" />
          <path d="M 0 0 C 70 40, 80 110, 110 170" strokeWidth="1.2" strokeLinecap="round" />
          {/* Leaves along vine */}
          <path d="M 18 35 Q 35 30 25 45 Q 12 45 18 35 Z" fill="currentColor" stroke="none" />
          <path d="M 32 85 Q 50 80 40 95 Q 26 95 32 85 Z" fill="currentColor" stroke="none" />
          <path d="M 36 140 Q 56 135 46 152 Q 30 150 36 140 Z" fill="currentColor" stroke="none" />
          <path d="M 68 70 Q 88 65 78 80 Q 60 80 68 70 Z" fill="currentColor" stroke="none" />
          <path d="M 90 125 Q 110 120 100 135 Q 82 135 90 125 Z" fill="currentColor" stroke="none" />
        </svg>
      </motion.div>

      {/* Right Hanging Vine Branch */}
      <motion.div
        style={{ rotate: rotateRight, transformOrigin: "top right" }}
        className="absolute top-0 right-0 w-64 h-80"
      >
        <svg className="w-full h-full text-[#1C2E20] transform scale-x-[-1]" viewBox="0 0 200 250" fill="none" stroke="currentColor">
          <path d="M 0 0 C 40 60, 20 120, 50 200" strokeWidth="1.8" strokeLinecap="round" />
          <path d="M 0 0 C 70 40, 80 110, 110 170" strokeWidth="1.2" strokeLinecap="round" />
          {/* Leaves along vine */}
          <path d="M 18 35 Q 35 30 25 45 Q 12 45 18 35 Z" fill="currentColor" stroke="none" />
...
function Navbar() {
  return (
    <nav className="sticky top-0 w-full z-50 bg-[#EFF4EC]/90 border-b border-[#1C2E20]/15 backdrop-blur-md shadow-xs">
